...
- 不提交账号、密码、手机号、IoT Token、设备 ID、抓包、APK 或 HA
  `.storage` 数据。
- 密码和 AppSecret 使用 HA 密码输入框采集，只保存在用户自己的
  Home Assistant 配置存储中。
...
- 心够智家账号手机号；
- 心够智家账号密码；
- 你有权使用的 Link Living AppKey；
- 对应的 Link Living AppSecret。
...
5. 使用 `/thing/properties/get` 和 `/thing/properties/set` 读取及控制设备。
